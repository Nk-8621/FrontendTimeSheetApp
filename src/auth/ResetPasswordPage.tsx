...
export function ResetPasswordPage({ identifier, onSuccess, onBackToLogin }: ResetPasswordPageProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setInfo('');
    try {
      const result = await authApi.resetPassword(identifier, otpCode.trim(), newPassword, confirmNewPassword);
  ...
  async function handleResend() {
    ...
    setInfo('');
    try {
      await authApi.resendPasswordResetOtp(identifier);
      setInfo('If an account exists for that identifier, a new code has been sent.');
      ...
      <form className={styles.card} onSubmit={handleSubmit}>
        ...
        <button
          ...
          style={{
            ...
            color: cooldownRemaining > 0 ? '#9AA7B4' : '#17456B',
            fontSize: 12.5, cursor: cooldownRemaining > 0 ? 'default' : 'pointer',
          }}
        >
